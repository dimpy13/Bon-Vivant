...
with open('cleaned_Recipe.csv', 'rb') as f:
    reader = csv.reader(f)
    recipes = list(reader)
    # print recipes
...
recipeComp=[[0]*1111 for i in range(23103)]
j = -1
for recipeLine in recipes:
    ...
    ingreList = recipeLine[2:]
    ingreNum = len(ingreList)
    flag = 0
    for _ingre in ingreList:
        # new_list = []
        # flag = 0
...
with open("recipe_Compound.csv", "wb") as f:
    writer = csv.writer(f)
