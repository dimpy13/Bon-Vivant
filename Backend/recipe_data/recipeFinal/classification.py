...
if __name__ == '__main__':
    ...
    yum_clean = pd.read_pickle('D:/Projects/Flavor-Network-master/Flavor-Network-master/data/yummly_clean.pkl')
    #print (yum_clean)

    #create a set of all ingredients in the dataframe
    yum_ingredients=set()
    #print(yum_ingredients)

    yum_clean['clean ingredients'].map(lambda x: [yum_ingredients.add(i) for i in x])
    #print(yum_clean['clean ingredients'])
    #print (yum_clean)
    ...
    plt.show()'''
    #read pickled dataframe
    yum_ingr = pd.read_pickle('D:/Projects/Flavor-Network-master/Flavor-Network-master/data/yummly_ingr.pkl')
    yum_tfidf = pd.read_pickle('D:/Projects/Flavor-Network-master/Flavor-Network-master/data/yum_tfidf.pkl')
    print (yum_tfidf)
    #plot confusion matrix for flavor-based classification
    #print(yum_ingr['cuisine'])
    X = yum_tfidf.values
    print (X)
    y = yum_ingr['cuisine']
    X_train, X_test, y_train, y_test = train_test_split(X,y,random_state=10)
